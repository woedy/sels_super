...
from candidates.api.serializers import AllParliamentaryCandidateSerializer
from candidates.models import Party, ParliamentaryCandidate
from ghana_decides_proj.exceptions import ClientError



class ParliamentaryCandidateConsumers(AsyncJsonWebsocketConsumer):

    async def connect(self):
        self.user = None
        self.room_group_name = "parliamentary-candidate-room"
        self.user_id = None
    ...
    async def receive_json(self, content):
        ...
        parl_can_id = content.get("parl_can_id", None)

        try:
            if command == "add_parliamentary_candidate":
                await self.add_parliamentary_candidate(user_id, data)

            if command == "get_all_parliamentary_candidates":
                await self.get_all_parliamentary_candidates(user_id, search, page)

            if command == "delete_parliamentary_candidate":
                await self.delete_parliamentary_candidate(user_id, parl_can_id)

            if command == "edit_parliamentary_candidate":
                await self.edit_parliamentary_candidate(user_id, parl_can_id, data)

        except ClientError as e:
            await self.handle_client_error(e)

    async def add_parliamentary_candidate(self, user_id, data):
        await self.channel_layer.group_add(
            self.room_group_name,
            self.channel_name
        )

        add_candidate_data = await add_parliamentary_candidate(user_id, data)
    ...
    async def get_all_parliamentary_candidates(self, user_id, search, page):
        await self.channel_layer.group_add(
            self.room_group_name,
            self.channel_name
        )

        get_all_candidates_data = await get_all_parliamentary_candidates(user_id, search, page)
    ...
    async def delete_parliamentary_candidate(self, user_id, parl_can_id):
        await self.channel_layer.group_add(
            self.room_group_name,
            self.channel_name
        )

        delete_candidate_data = await delete_parliamentary_candidate(user_id, parl_can_id)
    ...
    async def edit_parliamentary_candidate(self, user_id, candidate_id, data):
        await self.channel_layer.group_add(
            self.room_group_name,
            self.channel_name
        )

        edit_candidate_data = await edit_parliamentary_candidate(user_id, candidate_id, data)
...
def add_parliamentary_candidate(user_id, dataa):
    payload = {}
    data = {}
    errors = {}
...
@database_sync_to_async
def get_all_parliamentary_candidates(user_id, search_query, page):
    payload = {}
    data = {}
    errors = {}
    ...
    all_candidates = ParliamentaryCandidate.objects.all()
    ...
    candidates_serializer = AllParliamentaryCandidateSerializer(candidates, many=True)
...
@database_sync_to_async
def edit_parliamentary_candidate(user_id, candidate_id, data):
    # Implement the logic to edit a parliamentary candidate
    pass



@database_sync_to_async
def delete_parliamentary_candidate(user_id, parl_can_id):
    payload = {}
    errors = {}
