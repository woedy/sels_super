...
const ListAllPollingStationsSoc = () => {
    const [pollingStations, setPollingStation] = useState([]);
    ...
    useEffect(() => {
        const socket = new WebSocket(baseWsUrl + 'ws/polling-station-consumers/');
        
        socket.onopen = () => {
            console.log('WebSocket connected');

            const payload = {
                command: 'get_all_polling_stations',
                user_id: 'ewrwerfsd'
            };
        ...
        socket.onmessage = (event) => {
            ...
            if (data.payload && data.payload.message === 'Successful') {
                ...
                setPollingStation(data.payload.data || []);
            } else {
    ...
    function handleDelete(polling_station_id) {
        const socket = new WebSocket(baseWsUrl + 'ws/polling-station-consumers/');
        ...
        socket.onopen = () => {
            ...
            const payload = {
                command: 'delete_polling_station',
        ...
        socket.onmessage = (event) => {
            const data = JSON.parse(event.data);
            if (data.payload && data.payload.message === 'Successful') {
                console.log('Polling Station deleted successfully');
                // Perform any necessary actions after successful deletion
            } else {
                console.error('Failed to delete polling station');
            }
        };
                        ...
                        <div>
                            <div className='grid grid-cols-5'>
                                <div className='col-span-4'>
                                    <p className='text-white text-center text-2xl font-bold mb-3' style={{ textShadow: '0px 4px 4px rgba(0, 0, 0, 0.25)' }}>ALL POLLING STATIONS</p>
                                </div>
                                <div className='col-span-1'>
                                    <div className="rounded-full bg-blue-500 shadow-lg p-2 flex items-center justify-center cursor-pointer">
                                        <p className="text-white text-center text-sm">Add Polling Station</p>
                                    </div>
                            ...
                            <div>
                                <div className='grid gap-4 grid-cols-4 grid-rows-2'>
                                    {pollingStations.map((pollingStation) => (
                                        <div key={pollingStation.polling_station_id} className="relative">
                                            <Link to={"/polling-station-details/" + pollingStation.polling_station_id}>
                                                <div className="w-60 h-20 bg-black bg-opacity-25 backdrop-blur-lg rounded-lg flex items-center justify-center m-3 shadow-md">
                                                    <p className="text-white text-xl font-bold p-4 text-center" style={{ textShadow: '0px 4px 4px rgba(0, 0, 0, 0.25)' }}>{pollingStation.polling_station_name}</p>
                                                </div>
                                            </Link>
                                            ...
                                            <div className="absolute top-0 right-0 m-2">
                                                <div className="bg-red-500 rounded-full p-2 cursor-pointer">
                                                    <FaTimes className="text-white" onClick={() => handleDelete(pollingStation.polling_station_id)} />
                                                </div>
                                            </div>
                                        </div>
                                    ))}
...
export default ListAllPollingStationsSoc;
