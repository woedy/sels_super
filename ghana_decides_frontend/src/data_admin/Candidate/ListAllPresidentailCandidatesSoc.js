...
const ListAllPresidentialCandidatesSoc = () => {
    const [presidentialCandidates, setPresidentialCandidate] = useState([]);

    const navigate = useNavigate();


    useEffect(() => {
        const socket = new WebSocket(baseWsUrl + 'ws/presidential-candidate-consumers/');
        
        socket.onopen = () => {
            console.log('WebSocket connected');

            const payload = {
                command: 'get_all_presidential_candidates',
                user_id: 'ewrwerfsd'
            };
        ...
        socket.onmessage = (event) => {
            ...
            if (data.payload && data.payload.message === 'Successful') {
                ...
                setPresidentialCandidate(data.payload.data || []);
            } else {
    ...
    function handleDelete(prez_can_id) {
        const socket = new WebSocket(baseWsUrl + 'ws/presidential-candidate-consumers/');
        ...
        socket.onopen = () => {
            ...
            const payload = {
                command: 'delete_presidential_candidate',
                prez_can_id: prez_can_id
            };
        ...
        socket.onmessage = (event) => {
            const data = JSON.parse(event.data);
            if (data.payload && data.payload.message === 'Successful') {
                console.log('Presidentail Candidate deleted successfully');
                // Perform any necessary actions after successful deletion
            } else {
                console.error('Failed to delete presidential candidate');
            }
        };
                            ...
                            <div className='grid grid-cols-5'>
                                ...
                                <div className='col-span-1'>
                                    <div className="rounded-full bg-blue-500 shadow-lg p-2 flex items-center justify-center cursor-pointer">
                                        <p className="text-white text-center text-sm">Add Presidential Candidate</p>
                                    </div>
                            ...
                            <div>
                                <div className='grid gap-4 grid-cols-4 grid-rows-2'>
                                    {presidentialCandidates.map((candidate) => (
                                        <div key={candidate.prez_can_id} className="relative">
                                            <Link to={"/presidential-candidate-details/" + candidate.prez_can_id}>
                                            ...
                                            <div className="absolute top-0 right-0 m-2">
                                                <div className="bg-red-500 rounded-full p-2 cursor-pointer">
                                                    <FaTimes className="text-white" onClick={() => handleDelete(candidate.prez_can_id)} />
                                                </div>
                                            </div>
                                        </div>
                                    ))}
...
export default ListAllPresidentialCandidatesSoc;
